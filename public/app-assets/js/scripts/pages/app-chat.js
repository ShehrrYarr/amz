...
// Add message to chat
function chatMessagesSend(message, chat_id, chat_name) { 
	 
	var request = new XMLHttpRequest();
	request.open("GET", "/sendmessage/"+encodeURIComponent(message)+"/"+chat_id, false);
	request.send();
	console.log(request.response);
	var message = chatMessageSend.val();
	 
	if (message != "") {
		var html = '<div class="chat">'+'<div class="chat-message"><div class="chat-avatar"><a class="avatar m-0"><img src="https://eu.ui-avatars.com/api/?name='+chat_name+'&background=random" alt="avatar" height="36" width="36" /></a></div>' + "<p><br><br>" + message + "</p>" + "<div class=" + "chat-time" + ">3:35 AM</div></div></div>";
